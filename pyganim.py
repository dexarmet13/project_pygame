...
class PygAnimation(object):
    def __init__(self, frames, loop=True):
        ...
        self._state = (
            STOPPED  # The state is always either PLAYING, PAUSED, or STOPPED
        )
        ...
        self._playingStartTime = (
            0  # the time that the play() function was last called.
        )
        self._pausedStartTime = (
            0  # the time that the pause() function was last called.
        )
        ...
        if (
            ...
            for i in range(self.numFrames):
                ...
                assert type(frame[0]) in (str, pygame.Surface), (
                    "Frame %s image must be a string filename or a"
                    " pygame.Surface" % (i)
                )
                assert (
                    frame[1] > 0
                ), "Frame %s duration must be greater than zero." % (i)
                if type(frame[0]) == str:
    ...
    def scale2x(self):
        ...
        self._makeTransformedSurfacesIfNeeded()
        for i in range(len(self._images)):
            self._transformedImages[i] = pygame.transform.scale2x(
                self.getFrame(i)
            )
    ...
    def _surfaceMethodWrapper(self, wrappedMethodName, *args, **kwargs):
        ...
        for i in range(len(self._images)):
            methodToCall = getattr(
                self._transformedImages[i], wrappedMethodName
            )
    ...
    def _propGetState(self):
        if self.isFinished():
            self._state = (
                STOPPED  # if finished playing, then set state to STOPPED.
            )
    ...
    currentFrameNum = property(
        _propGetCurrentFrameNum, _propSetCurrentFrameNum
    )
...
class PygConductor(object):
    def __init__(self, *animations):
        assert (
            len(animations) > 0
        ), "at least one PygAnimation object is required"
